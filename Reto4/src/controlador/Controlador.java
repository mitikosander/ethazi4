			...
			public void actionPerformed(ActionEvent arg0) {
				...
				vista.getListahoteles().getTable().setModel(modelos);
					...
					if(vista.getInicio().getRdbtnApartamento().isSelected()) {
						...
						//Usamos el metodo que cargara los hoteles de la ubicacion seleccionada
						DefaultTableModel modelos=modelo.getMetodos().cargarTablacasasrurales((String) vista.getInicio().getCombo_ubicacion().getSelectedItem());
				...
				// Tras la busqueda inicial vamos a la pantall de seleccion de hoteles
				  try
	                {                    
	                    ...
	                    if (vista.getInicio_sesion().getTf_Usuario_Inicio_Sesion().getText().length() > 0 && vista.getInicio_sesion().getPf_Contra_Inicio_Sesion().getText().length() > 0 )
	                    {
	                    	String contrasena = modelo.getMetodos().encriptarPass(vista.getInicio_sesion().getPf_Contra_Inicio_Sesion().getText());
	                        // Si el usuario si fue validado correctamente
	                    	System.out.println(contrasena);
	                        if( modelo.getMetodos().comprobarLogin( vista.getInicio_sesion().getTf_Usuario_Inicio_Sesion().getText(), vista.getInicio_sesion().getPf_Contra_Inicio_Sesion().getText() ) )    //enviar datos a validar
	                        {
	                        ...
	                        else
	                        {
	                            ...
	                            JOptionPane.showMessageDialog(null, vista.getInicio_sesion().getTf_Usuario_Inicio_Sesion().getText()+" " +vista.getInicio_sesion().getPf_Contra_Inicio_Sesion().getText() );
